...
class TestCrawlerSection:
    ...
    def test_override_fields(self):
        cfg = CrawlerSection(request_delay=5.0, max_retries=1)
        assert cfg.request_delay == 5.0
...
class TestAppConfig:
    def test_defaults(self):
        cfg = AppConfig()
        assert cfg.crawler.request_delay == 2.5
        assert cfg.analysis.engine == "pandas"
        assert cfg.data.raw_dir == "data/raw"

    def test_nested_override(self):
        cfg = AppConfig(
            crawler=CrawlerSection(request_delay=5.0),
...
class TestLoadConfig:
    ...
    def test_loads_from_yaml_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        yaml_content = {
            "crawler": {"request_delay": 8.0, "max_retries": 10},
        ...
        assert cfg.crawler.request_delay == 8.0
        assert cfg.crawler.max_retries == 10
        assert cfg.crawler.retry_delay == 1.0  # default preserved
        assert cfg.analysis.engine == "spark"
    ...
    def test_partial_yaml_preserves_defaults(self, tmp_path, monkeypatch):
        ...
        cfg = load_config(str(config_path))
        assert cfg.crawler.request_delay == 4.0
        assert cfg.crawler.max_retries == 3  # default

    def test_env_var_overrides_yaml(self, tmp_path, monkeypatch):
        """环境变量覆盖 YAML 文件设置"""
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("BILI_CRAWLER__REQUEST_DELAY", "3.0")

        config_path = tmp_path / "config.yaml"
        config_path.write_text("crawler:\n  request_delay: 5.0\n", encoding="utf-8")

        cfg = load_config(str(config_path))
        assert cfg.crawler.request_delay == 3.0
    ...
    def test_bili_config_path_env(self, tmp_path, monkeypatch):
        ...
        cfg = load_config()
        assert cfg.crawler.max_retries == 7
    ...
    def test_numeric_env_var_casting(self, tmp_path, monkeypatch):
        ...
        monkeypatch.chdir(tmp_path)
        monkeypatch.setenv("BILI_CRAWLER__MAX_RETRIES", "20")
        monkeypatch.setenv("BILI_CRAWLER__REQUEST_DELAY", "5.0")

        cfg = load_config("nonexistent.yaml")
        assert cfg.crawler.max_retries == 20
        assert cfg.crawler.request_delay == 5.0
    ...
    def test_parameter_overrides_env(self, tmp_path, monkeypatch):
        ...
        assert cfg.crawler.max_retries == 3
